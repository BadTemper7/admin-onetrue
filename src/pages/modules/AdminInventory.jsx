    ...
    <div className="space-y-6">
      ...
      <div className="card overflow-hidden">
        ...
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-bold text-slate-500">
          Showing {filteredContainers.length} of {bookingContainers.length} assigned containers
        </div>
          ...
          <table className="w-full min-w-[1180px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredContainers.length === 0 && !loading && (
                <tr>
                  <td colSpan="7" className="px-5 py-10 text-center font-bold text-slate-500">No assigned booking containers found.</td>
                </tr>
              )}
                      ...
                      <div className="flex flex-col items-stretch gap-2 sm:items-end">
                        {canMarkStored && (
                          <button type="button" onClick={() => handleMarkStored(container)} className="btn-primary whitespace-nowrap">
                            <PackageCheck size={16} /> Mark Stored
                          </button>
                        )}
                        {canRelocate && (
